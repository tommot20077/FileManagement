...
/**
 * Redis 請求限制器過濾器的單元測試類別。
 *
 * <p>本測試類別全面驗證 RedisRequestLimiterFilter 在 Redis 環境下的請求限制和效能管理機制。</p>
 *
 * <p>測試範圍：
 * 
 *   - Redis 分散式請求限制機制
...
class RedisRequestLimiterFilterTest {
    ...
    void filter_NormalCase_RequestNotExceedLimit() {
        ...
        try (MockedStatic<LogUnity> mockedLogUnity = mockStatic(LogUnity.class); MockedStatic<ClientIpFilter> mockedClientIpFilter = mockStatic(
            ...
            mockedClientIpFilter.when(() -> ClientIpFilter.getClientIpFromExchange(any(ServerWebExchange.class))).thenReturn(Optional.of(TEST_IP));

            RedisRequestLimiterFilter localRedisRequestLimiterFilter = spy(new RedisRequestLimiterFilter(globalProperties,
                                                                                                         objectMapper,
                                                                                                         redisClient,
                                                                                                         redisProvider
            ));
            ...
            doReturn(Mono.empty())
                    .when(localRedisRequestLimiterFilter)
                    .sendErrorResponse(any(ServerWebExchange.class), any(ObjectMapper.class), any(ValidationException.ErrorCode.class), any());
            doReturn(Mono.empty())
                    .when(localRedisRequestLimiterFilter)
                    .sendErrorResponse(any(ServerWebExchange.class), any(ObjectMapper.class), any(LimitationException.ErrorCode.class), anyString());
    ...
    void filter_ExceptionCase_CannotGetClientIp() {
        try (MockedStatic<LogUnity> mockedLogUnity = mockStatic(LogUnity.class); MockedStatic<ClientIpFilter> mockedClientIpFilter = mockStatic(
            ...
            mockedClientIpFilter.when(() -> ClientIpFilter.getClientIpFromExchange(any(ServerWebExchange.class))).thenReturn(Optional.empty());
            ...
            doReturn(Mono.empty())
                    .when(localRedisRequestLimiterFilter)
                    .sendErrorResponse(any(), any(), any(ValidationException.ErrorCode.class), any());

            StepVerifier.create(localRedisRequestLimiterFilter.filter(exchange, chain)).verifyComplete();

            verify(response).setStatusCode(HttpStatus.BAD_REQUEST);
            verify(localRedisRequestLimiterFilter).sendErrorResponse(eq(exchange),
                                                                     eq(objectMapper),
                                                                     eq(ValidationException.ErrorCode.REQUEST_IS_INVALID),
                                                                     eq("IP 地址")
            );
    ...
    void filter_ExceptionCase_RedisServiceError() {
        ...
        try (MockedStatic<LogUnity> mockedLogUnity = mockStatic(LogUnity.class); MockedStatic<ClientIpFilter> mockedClientIpFilter = mockStatic(
            ...
            doReturn(Mono.empty())
                    .when(localRedisRequestLimiterFilter)
                    .sendErrorResponse(any(), any(), any(LimitationException.ErrorCode.class), anyString());
    ...
    void filter_BoundaryCase_IpBanActivated() {
        ...
        try (MockedStatic<LogUnity> mockedLogUnity = mockStatic(LogUnity.class); MockedStatic<ClientIpFilter> mockedClientIpFilter = mockStatic(
            ...
            verify(localRedisRequestLimiterFilter).sendErrorResponse(eq(exchange),
                                                                     eq(objectMapper),
                                                                     eq(ValidationException.ErrorCode.ALREADY_BAN_IP)
            );
            verify(chain, never()).filter(exchange);
        }
    }
    ...
    void constructor_BoundaryCase_DefaultLimitAndRefill() {
        ...
        try (MockedStatic<LogUnity> mockedLogUnity = mockStatic(LogUnity.class); MockedStatic<ClientIpFilter> mockedClientIpFilter = mockStatic(
            ...
            RedisRequestLimiterFilter localRedisRequestLimiterFilter = new RedisRequestLimiterFilter(localGlobalProperties,
                                                                                                     localObjectMapper,
                                                                                                     localRedisClient,
                                                                                                     localRedisProvider
            );
